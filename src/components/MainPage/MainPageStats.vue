...
<template>
    <section class="flex flex-col items-center justify-between gap-10 border-t pt-8 lg:flex-row lg:gap-8">
        ...
        <div class="-mx-6 grid grid-cols-2 gap-4 md:-mx-8 md:flex md:divide-x">
            <div class="px-6 md:px-8" v-for='stat in statsData' :key='stat.id'>
                <span
                    class="block text-center text-lg font-bold text-foreground-muted md:text-left md:text-xl">{{ stat.count }}</span>
                <span class="block text-center text-sm font-semibold text-gray-700 md:text-left md:text-base">
                    {{ stat.title }}
                </span>
            </div>
        </div>
